...
def score(f, ref, uem, sys):
    der_score = "perl ~/Documents/score/md-eval.pl -as -c 0.25 -r %s %s -s %s > %s.output.der" % (ref, uem, sys, sys)
...
def findboundary(ref, sys, stat):
    ...
    output = commands.getoutput("/home/zzpp220/Matlab/bin/matlab -nodesktop -nojvm -nosplash -nodisplay '%s' > %s.output" % (mscript, sys) )
    overall_score = 0
    nmatches = 0
    nins = 0
    ndel = 0
...
# ------------------------------------------------
print "------------------------------------------------"
#print "USAGE: python score.py -r /path/to/referencelist/file -s /path/to/sysrttm/folder/ (-u /path/to/uem/folder/) (-dp) (-pu)"
#print "------------------------------------------------"
#print " "
mode = ""
if "-r" in sys.argv:
